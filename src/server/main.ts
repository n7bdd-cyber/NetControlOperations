 ...
 * File: main.ts
 * System Version: 1.0.0 | File Version: 13 | Date: 2026-05-15
 *   v13: S5-12 — getWinlinkPracticeMessage() generates Winlink practice message.
...
// ---------------------------------------------------------------------------
// S5-12 — getWinlinkPracticeMessage: builds a formatted Winlink practice
// message for display on screen during an open session. Read-only, no lock.
// ---------------------------------------------------------------------------

const WINLINK_MONTH_ABBREVS = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];

export function getWinlinkPracticeMessage(sessionId: string): WinlinkMessageResult {
  if (!sessionId || !isValidIdField(sessionId, MAX_ID_FIELD)) {
    return { ok: false, error: 'INVALID_INPUT', field: 'sessionId', reason: 'missing or too long' };
  }
  ...
  try {
    ...
    Logger.log(`getWinlinkPracticeMessage: sessions read threw: ${String(e)}`);
    return { ok: false, error: 'NOT_CONFIGURED' };
  }
  if (!sessRow) return { ok: false, error: 'SESSION_NOT_FOUND' };
  if (String(sessRow[SessionsCol.Status]) !== SESSION_STATUS_OPEN) {
  ...
  const monthIdx   = parseInt(dateParts[1] || '1', 10) - 1;
  const dayInt     = parseInt(dateParts[2] || '1', 10);
  const monthAbbr  = WINLINK_MONTH_ABBREVS[monthIdx] ?? 'Jan';

  const dateFormatted      = `${dayInt} ${monthAbbr} ${year}`;            // "15 May 2026"
  const rfcDate            = `${dayInt} ${monthAbbr} ${year} ${netTime} -0700`;
  const ncoLocationDisplay = ncoLocation.trim() || 'QTH unknown';

  const to      = 'W6BA@winlink.org';
  const from    = ncoCallsign.toUpperCase() + '@winlink.org';
  const subject = `Practice — ${netType} — ${netDate}`;

  const body = [
    `This is a Winlink practice message from the ${netType}.`,
    '',
    `Net date: ${dateFormatted}`,
    `Net control: ${ncoCallsign} (${ncoLocationDisplay})`,
    '',
  ...
  const formattedText = [
    ...
    `  3. Set the To address to: ${to}`,
    '  4. Copy the Subject and Body above.',
    '  5. Send via your preferred Winlink gateway.',
  ].join('\n');

  const winlinkMessage: WinlinkMessage = { to, cc: '', from, subject, date: rfcDate, body, formattedText };
  return { ok: true, message: winlinkMessage };
}
