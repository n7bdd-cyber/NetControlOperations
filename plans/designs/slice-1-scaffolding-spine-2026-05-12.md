...
- **esbuild bundle footer — switched from property assignment to explicit function declarations on 2026-05-13.** The original IIFE-with-footer used `for (var k in __app__) { this[k] = __app__[k]; }`. That works at runtime (the functions ARE callable via `google.script.run` and as web-app entry points), but Apps Script's script editor populates its "Run" dropdown by **static** parsing of the deployed code, so the dropdown came up empty and `setupSheets()` had no human-triggerable entry. `scripts/build.mjs` now emits five explicit `function name() { return __app__.<name>.apply(this, arguments); }` declarations in the footer, one per entry point. **Remaining hygiene follow-up:** add a post-build smoke test (next time `scripts/build.mjs` is touched, or when a sixth entry point is introduced) that reads `dist/Code.gs` and asserts each expected `function <name>(` declaration appears — this would have caught the empty-dropdown problem at build time rather than at deploy time.
